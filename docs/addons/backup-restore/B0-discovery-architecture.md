...
**Resolved host contract.** B1H retains the approved `Alta\BackupRestore\BackupRestoreServiceProvider` identity through a generic standalone-package mode: the existing manifest provider is resolved through validated package-local `composer.json` PSR-4 metadata, canonical confinement and reserved/collision policy. Bundled `Modules\...`/`Extensions\...` behavior remains backward-compatible; no addon-specific core exception or redundant namespace manifest field is used.
